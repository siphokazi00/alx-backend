...
class Server:
    ...
    def get_hyper_index(self, index: int = None, page_size: int = 10) -> Dict[str, Any]:
        ...
        """
        assert index is not None and isinstance(index, int) and 0 <= index < len(self.dataset()), "Index out of range"

        indexed_dataset = self.indexed_dataset()
        data = []
        current_index = index

        # Iterate until we fill the page or reach the end of the dataset
